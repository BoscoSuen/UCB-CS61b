...
public class DList2 {
  ...
  public void insertFront(int i) {
	...
	if (head.prev == head) {					// if the list is empty.
	  head.next = node;
	  head.prev = node;							// we also need to set node point to head, the default is null.
	  node.prev = head;
	  node.next = head;
	  size ++;
	} else {
	  ...
	  node.next = head.next;
	  node.prev = head;
	  // now we need to remove the old connection:
	  head.next.prev = node;
	  head.next = node;
	  size ++;
	}
  ...
  public void removeFront() {
	...
	if (this.size != 0) {
	  ...
	  head.next.prev = head;		// the second-first node points to the sentinel.
	  size --;
	} else {
