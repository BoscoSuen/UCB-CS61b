...
public class DList1 {
  ...
  public void insertFront(int i) {
    // Your solution here.
	if (this.size != 0) {
      DListNode1 node = new DListNode1(i);			// default node.prev = null.
	  node.next = head;
	  head.prev = node;			// while inserting, do not need to worry about head.next.
  ...
  public void removeFront() {
	...
	if(this.size != 0) {
	  if (this.size != 1) {							
		head.next = null;							// remove the head.
		tail.prev = null;							// tail becomes to the new head;
		head = tail;
		size --;
	  } else {										// while removing, need to worry about the new head&tail when size = 1.
		head = null;
		tail = null;
		size --;									// remove the head.
	  }
	} else {
	  System.out.println("The size of the list = 0.");
	}
  }
