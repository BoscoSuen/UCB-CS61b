  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
  	if (isValidNode()) {
    	...
    	if (kid == null) {
    		kid = new SibTreeNode(this.myTree, item);
    		kid.parent = this;
    		this.firstChild = kid;
    	} else if (c == 1) {
    		SibTreeNode node = new SibTreeNode(this.myTree, item);
    		node.parent = this;
    		this.firstChild = node;
    		node.nextSibling = kid;
    	} else if (c == 2) {
    		SibTreeNode node = new SibTreeNode(this.myTree, item);
    		node.parent =this;
    		node.nextSibling = kid.nextSibling;
  	...
  	if (this.isValidNode()) {
  		if (this.firstChild == null) {
  			if (this.parent == null) {
    			...
    			SibTreeNode p = this.parent;
    			SibTreeNode current = p.firstChild;
    			if (current == this) {
    				p.firstChild = this.nextSibling;
    				this.nextSibling = null;
    				this.parent = null;
    				this.valid = false;
    				myTree.size--;
    			} else {									// do not need to change the firstChild.
     			 while ( current.nextSibling != null) {
     				 if (current.nextSibling == this) {
